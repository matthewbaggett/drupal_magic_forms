...
!function ($) {
    ...
    var SpinEdit = function (element, options) {
        ...
        template.find('.spinedit-up').mousehold($.proxy(this.increase, this));
        template.find('.spinedit-down').mousehold($.proxy(this.decrease, this));
    ...
    DRPGlobal.template =
        '<div class="spinedit">' +
            '<i class="glyphicon glyphicon-play spinedit-up"></i>' +
            '<i class="glyphicon glyphicon-play spinedit-down"></i>' +
            '</div>';
